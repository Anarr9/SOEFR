        ...
        public async Task Disconnect()
        {
            if (clientWebSocket != null && clientWebSocket.State == WebSocketState.Open)
            {
                try
                {
                    ...
                    Debug.WriteLine("Disconnected from the WebSocket server.");
                    ConnectionStatus = "Disconnected from the WebSocket server.";
                    ConnectionStatusChanged?.Invoke(ConnectionStatus);
                }
                catch (WebSocketException ex)
                {
                    Debug.WriteLine($"WebSocket error during disconnect: {ex.Message}");
                    ConnectionStatus = $"WebSocket error during disconnect: {ex.Message}";
                    ConnectionStatusChanged?.Invoke(ConnectionStatus);
                }
            }
        }
